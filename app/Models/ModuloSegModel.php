...
class ModuloSegModel
{
    ...
    {
        $res = $this->request('GET', 'modulos_seg?select=*&order=id.asc');
        return ($res['code'] === 200 && is_array($res['body'])) ? $res['body'] : [];
    }

    public function obtenerPorId(int $id): array
    {
        $res = $this->request('GET', 'modulos_seg?id=eq.' . $id . '&limit=1');
        return ($res['code'] === 200 && !empty($res['body'])) ? $res['body'][0] : [];
    }

    public function crear(array $data): bool
    {
        $res = $this->request('POST', 'modulos_seg', $data, ['Prefer: return=minimal']);
        return $res['code'] === 201;
    }

    public function actualizar(int $id, array $data): bool
    {
        $res = $this->request('PATCH', 'modulos_seg?id=eq.' . $id, $data);
        return in_array($res['code'], [200, 204]);
    }

    public function eliminar(int $id): bool
    {
        $res = $this->request('DELETE', 'modulos_seg?id=eq.' . $id);
        return in_array($res['code'], [200, 204]);
    }
}
