...
$puedeAgregar  = $isAdmin || !empty($permisos[6]['bitAgregar']);
$puedeEditar   = $isAdmin || !empty($permisos[6]['bitEditar']);
$puedeEliminar = $isAdmin || !empty($permisos[6]['bitEliminar']);
$puedeDetalle  = $isAdmin || !empty($permisos[6]['bitDetalle']);
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="fw-bold mb-1">
            <i class="bi <?= esc($icono ?? 'bi-people-fill') ?> me-2 text-<?= esc($color ?? 'success') ?>"></i><?= esc($titulo ?? 'Clientes y Leads') ?>
        </h2>
        <p class="text-muted small mb-0"><?= esc($subtitulo ?? 'Base de datos de clientes potenciales y actuales') ?></p>
    </div>
                ...
                <tbody>
                    <tr>
                        <td colspan="7" class="text-center py-5 text-muted">
                            ...
                            <strong>Sin clientes ni leads registrados</strong>
