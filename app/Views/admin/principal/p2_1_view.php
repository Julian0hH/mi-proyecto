...
$puedeAgregar  = $isAdmin || !empty($permisos[7]['bitAgregar']);
$puedeEditar   = $isAdmin || !empty($permisos[7]['bitEditar']);
$puedeEliminar = $isAdmin || !empty($permisos[7]['bitEliminar']);
$puedeDetalle  = $isAdmin || !empty($permisos[7]['bitDetalle']);
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="fw-bold mb-1">
            <i class="bi <?= esc($icono ?? 'bi-kanban-fill') ?> me-2 text-<?= esc($color ?? 'warning') ?>"></i><?= esc($titulo ?? 'Gestión de Proyectos') ?>
        </h2>
        <p class="text-muted small mb-0"><?= esc($subtitulo ?? 'Seguimiento de tareas, entregables y equipos de trabajo') ?></p>
    </div>
                ...
                <tbody>
                    <tr>
                        <td colspan="7" class="text-center py-5 text-muted">
                            ...
                            <strong>Sin proyectos en gestión</strong>
