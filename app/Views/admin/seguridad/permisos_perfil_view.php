...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted small mb-0">Asigna acciones CRUD a cada perfil por página protegida</p>
    </div>
</div>
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-transparent border-0 pt-3 px-4">
                <h6 class="fw-bold mb-0"><i class="bi bi-shield-check me-2 text-primary"></i>Seleccionar Perfil</h6>
            </div>
    ...
    <div class="col-md-8">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-transparent border-0 pt-3 px-4 d-flex align-items-center justify-content-between">
                <h6 class="fw-bold mb-0"><i class="bi bi-table me-2 text-warning"></i>Permisos por Página</h6>
                <?php if ($puedeEditar): ?>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover mb-0 align-middle" id="tabla-permisos">
                        <thead>
                            <tr>
                                <th class="px-4">Página / Módulo</th>
...
document.getElementById('sel-perfil').addEventListener('change', async function() {
    ...
    try {
        const res  = await fetch(`${BASE_PERMISOS}/cargar/${perfilId}`, {headers:{'X-Requested-With':'XMLHttpRequest'}});
...
function renderTabla() {
    const tbody = document.getElementById('tabla-body');
    if (!modulos.length) {
        tbody.innerHTML = '<tr><td colspan="6" class="text-center py-3 text-muted">Sin páginas registradas</td></tr>';
        return;
    }
    ...
    for (const [grupo, items] of Object.entries(grupos)) {
        html += `<tr class="table-light"><td colspan="6" class="px-4 py-1 fw-semibold small text-muted">${escHtml(grupo)}</td></tr>`;
        items.forEach(mod => {
            const p    = permisosCargados[mod.id] || {};
            const bits = ['bit_consulta','bit_agregar','bit_editar','bit_eliminar','bit_detalle'];
            const lbls = ['consulta','agregar','editar','eliminar','detalle'];
            const checks = bits.map((bit, i) => `
                <td class="text-center">
                    <div class="form-check d-flex justify-content-center mb-0">
                        <input class="form-check-input" type="checkbox" name="mod_${mod.id}_${lbls[i]}"
                            id="chk_${mod.id}_${lbls[i]}" ${p[bit] ? 'checked' : ''}>
                    </div>
                </td>`).join('');
            html += `<tr>
                <td class="px-4 fw-semibold">
                    <i class="${escHtml(mod.icono||'bi-circle')} me-2 text-muted"></i>${escHtml(mod.nombre)}
                    <small class="text-muted d-block fw-normal" style="font-size:.75em"><code>${escHtml(mod.ruta)}</code></small>
                </td>
                ${checks}
            </tr>`;
        });
...
document.getElementById('btn-guardar')?.addEventListener('click', async function() {
    const perfilId = document.getElementById('sel-perfil').value;
    if (!perfilId) return;
